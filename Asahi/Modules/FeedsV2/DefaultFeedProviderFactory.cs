...
{
    public class DefaultFeedProviderFactory(IHttpClientFactory httpClientFactory, DanbooruUtility danbooruUtility, IRedditApi redditApi, IDanbooruApi danbooruApi, BotConfig config) : IFeedProviderFactory
    {
        public IFeedProvider? GetFeedProvider(string feedSource)
        {
            ...
            if (CompiledRegex.RedditFeedRegex().IsMatch(feedSource))
                return new RedditFeedProvider(redditApi, config);
