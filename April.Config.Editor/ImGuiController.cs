...
namespace April.Config.Editor
{
    ...
    public sealed class ImGuiController : IDisposable
    {
        ...
        // Font shit
        // ~Kiev
        private IntPtr _fontArrayPtr;
        private IntPtr _iconsArrayPtr;
        private IntPtr _emojiArrayPtr;
        private ImFontPtr _fontPtr;

        public ImFontPtr FontPtr => _fontPtr;
        ...
        public void InitializeFonts()
        {
            unsafe
            {
                ...
                var font = GetEmbeddedResourceBytes("font.ttf");

                _fontArrayPtr = Marshal.AllocHGlobal(font.Length);
                Marshal.Copy(font, 0, _fontArrayPtr, font.Length);
                ...
                try
                {
                    _fontPtr = io.Fonts.AddFontFromMemoryTTF(_fontArrayPtr, font.Length, fontSize, fontConfig,
                ...
                fontConfig.GlyphOffset = new Vector2(0, 3f);
                ...
                var iconFont = GetEmbeddedResourceBytes("codicon.ttf");

                _iconsArrayPtr = Marshal.AllocHGlobal(iconFont.Length);
                Marshal.Copy(iconFont, 0, _iconsArrayPtr, iconFont.Length);
                try
                {
                    _fontPtr = io.Fonts.AddFontFromMemoryTTF(_iconsArrayPtr, iconFont.Length, fontSize, fontConfig, rangeHandle.AddrOfPinnedObject());
                }
                finally
                {
                    fontConfig.Destroy();
                ...
                fontConfig.PixelSnapH = true;
                fontConfig.GlyphOffset = new Vector2(0, 3f);
                var builder = (uint)ImGuiFreeTypeBuilderFlags.LoadColor;
                fontConfig.FontBuilderFlags = builder;

                rangeHandle = GCHandle.Alloc(new uint[]
                {
                    0x1,
                    0x1FFFF,
                    0
                ...
                var emojiFont = GetEmbeddedResourceBytes("fontemoji.ttf");

                _emojiArrayPtr = Marshal.AllocHGlobal(emojiFont.Length);
                Marshal.Copy(emojiFont, 0, _emojiArrayPtr, emojiFont.Length);
                try
                {
                    _fontPtr = io.Fonts.AddFontFromMemoryTTF(_emojiArrayPtr, emojiFont.Length, fontSize, fontConfig, rangeHandle.AddrOfPinnedObject());
                }
                finally
                {
                    fontConfig.Destroy();
        ...
        public void DestroyImGuiBinding(TextureView textureView)
        {
        ...
        public void Dispose()
        {
            ...
            Marshal.FreeHGlobal(_fontArrayPtr);
            Marshal.FreeHGlobal(_iconsArrayPtr);
